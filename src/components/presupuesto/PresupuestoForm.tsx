...
type FormValues = z.infer<typeof presupuestoSchema>;
...
export const PresupuestoForm: React.FC<PresupuestoFormProps> = ({ 
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      let result;
      if (initialData?.id) {
        result = await updatePresupuesto(initialData.id, presupuestoData);
      } else {
        result = await createPresupuesto(presupuestoData);
      }

      if (result) {
        onSuccess?.(); // Solo se cierra después de esta ejecución
        console.log('Presupuesto guardada exitosamente');
      } else {
    ...
    <Box component="form" onSubmit={handleSubmit(onSubmit)} sx={{ mt: 1 }}>
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <PaidIcon 
                    sx={{ 
                      color: cat.color,
                      presuform: cat.tipo === 'G' ? 'rotate(180deg)' : 'none' 
                    }} 
                  />
                  {cat.nombre}
                </Box>
      ...
      <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 2 }}>
        <TextField
          ...
          inputProps={{
            pattern: "[0-9]{4}-[0-9]{2}", // Esto fuerza el formato YYYY-MM
          }}
          ...
          {...register('periodo', {
            ...
            pattern: {
              value: /^[0-9]{4}-[0-9]{2}$/,
              message: 'El formato debe ser YYYY-MM'
            }
          })}
        />
      </Box>
